...
NodeChildIter *create_child_iter(Node *parent) {
	NodeChildIter *iter = (NodeChildIter *)malloc(sizeof(NodeChildIter));
	if (iter == NULL) {
		perror("Failed to allocate Node Iterator");
		return NULL;
	}
	iter->parent = parent;
	iter->current = parent->first_child;
	iter->first_child = parent->first_child;
	iter->last_child = parent->last_child;
	iter->current = parent->first_child;
	iter->current_child_index = 0;
	return iter;
}
Node *start_iter(NodeChildIter *iter) {
	iter->current = iter->first_child;
	iter->current_child_index = 0;
	return iter->current;
}
Node *end_iter(NodeChildIter *iter) {
	iter->current = iter->last_child;
	iter->current_child_index = iter->parent->number_of_children - 1;
	return iter->current;
}
...
Node *prev_iter(NodeChildIter *iter) {
	if (iter->current)
		iter->current = iter->current->prev_sibling;
	return iter->current;
}
